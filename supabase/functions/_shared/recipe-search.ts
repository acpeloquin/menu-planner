...
// d'appels doit rester faible. Assez de recherches pour comparer plus d'une
// recette candidate sur les sites de référence avant de se rabattre sur une
// recette composée par l'IA, sans risquer le timeout. Utilisé pour un repas à
// la fois (régénération) et pour generate-menu (plafonné à
// MAX_PARALLEL_GROUNDINGS repas en parallèle — voir generate-menu/index.ts —
// donc pas besoin d'un budget par appel plus léger : le plafond de repas est
// ce qui protège du timeout, pas un budget de recherche restreint).
