...
export const scrapeMaxi: ScrapeStore = async () => {
  const { images, validFrom, validTo } = await fetchCirculairesFlyerPages('maxi');
  return extractDealsFromFlyerImages(images, 'Maxi', validFrom, validTo);
};
